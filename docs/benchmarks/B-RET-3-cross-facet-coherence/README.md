...
**Goal:** quantify the `recall(facet_types=all)` bundle-assembly latency — the cross-facet T-shape retrieval that is the load-bearing primitive of Tessera's v0.1 product. The DoD target per `docs/release-spec.md §v0.1 DoD` is **median MCP `recall` latency < 500 ms with 10 000 facets in vault, all-local mode, on the reference hardware baseline** (MacBook Pro M1 Pro 10-core CPU / 16-core GPU, 16 GB RAM, macOS 15.x, daemon idle except for the test query, no concurrent model workload).

**Scope at the first baseline:**

- 2 000 facets across the five v0.1 facet types (identity, preference, workflow, project, style). The full 10 000-facet scale with real fastembed ONNX adapters is P12 territory.
...
P12 runs this at 10 000 facets with the fastembed ONNX embedder + reranker to validate the DoD target against production adapters on the reference hardware baseline.
