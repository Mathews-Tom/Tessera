...
side embedding latency so later numbers against the fastembed ONNX stack
at 10K and 100K facets attribute the delta to the provider, not to the
storage or the pipeline itself.
...
except for the test query, no concurrent model workload). This first
...
# DoD measurement: fastembed ONNX adapter stack at 10K, k=20
uv run python docs/benchmarks/B-RET-2-recall-latency/run.py \
...
- `inputs` — facet count, dim, trial count, embedder / reranker
  identifiers, retrieval mode, and `rerank_k`.
