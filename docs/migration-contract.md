...
6. `add_audit_chain_columns` — `ALTER TABLE audit_log ADD COLUMN prev_hash TEXT NOT NULL DEFAULT ''` and the same for `row_hash`. Both default to the empty string so existing rows acquire chain placeholders without blocking the column add. Guarded by `PRAGMA table_info` checks. (V0.5-P8, ADR 0021.)
7. `backfill_audit_chain` — walks every existing row in `id ASC` order, computes the forward-only hash chain in place via `tessera.vault.audit_chain.compute_row_hash`, and writes the populated `prev_hash` / `row_hash` columns. Idempotent: rows that already carry a non-empty `row_hash` are skipped, so an interrupted backfill replays cleanly. **Pre-upgrade rows are chained at upgrade in their stored order — pre-upgrade tampering is not retroactively detectable** (ADR 0021 §Security claim — exact boundary). `tessera audit verify` reports the genesis row id so users see which rows predate the chain. (V0.5-P8, ADR 0021.)
