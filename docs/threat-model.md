...
| Query and embedding content sent to cloud provider    | Information disclosure | Cloud adapter forwards facet content to T7 for embedding or reranking | Explicit per-adapter consent on enable. All-local mode (Ollama + sentence-transformers) is the default. Docs: "Cloud adapter sends the following content to the provider..." | v0.1   |
| Outbound traffic beyond configured adapters           | Information disclosure | Dependency makes unexpected network call (telemetry, update check)    | CI network-policy test: run full test suite with all outbound blocked except Ollama localhost; assert pass.                                                                  | v0.1   |
