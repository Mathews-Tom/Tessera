...
The compiler-orchestration CLI ships as `tessera playbook`. The six subcommands are `targets`, `sources`, `scaffold`, `register`, `stale`, and `inspect`: enumerate target descriptors, list eligible sources, emit a deterministic Markdown brief for an external compiler, register the compiled artifact, surface stale artifacts with their cascade cause, and read one field of a registered artifact (a Markdown heading or a `field_provenance` key, optionally with the matching provenance entry attached). There is intentionally no `tessera playbook compile` — Tessera stores; the caller compiles. The full command contract lives in `docs/api.md §CLI: tessera playbook`, and the documented runner workflows that wrap the CLI (Claude Code, local LLM, manual authoring) live in `docs/playbook-compiler-recipes.md`.
...
This is a **cross-facet** coherence claim, not a full **cross-session** coherence claim. Cross-facet coherence means the bundle returned by one `recall` call hangs together across identity, preference, workflow, project, and style. Cross-session coherence means the same user's operating model for the same project stays coherent across captures separated by weeks or months. v0.1 gets part of that from stable typed facets, but the richer cross-session property is deferred to the v0.5 episodic/temporal layer.
