...
- [x] **BYO sync round-trip preserves audit-chain integrity (V0.5-P9 part 1).** New `tessera/sync/` package with envelope encryption, signed monotonic manifest, BlobStore protocol + filesystem implementation, and snapshot-based push/pull primitives. The exit-gate scenario (`test_push_pull_round_trip_preserves_chain` in `tests/integration/test_sync_round_trip.py`) bootstraps a populated source vault, pushes through `LocalFilesystemStore` to a separate path, pulls on a fresh target, re-opens under the master key, runs `verify_chain`, and asserts the restored head matches the manifest's signed `audit_chain_head`. The S3-compatible adapter and `tessera sync` CLI are deferred to V0.5-P9b — the filesystem-backed store inherits-by-protocol every crypto and manifest invariant the S3 adapter will use, and is itself useful for filesystem-synced backup targets (iCloud Drive, Dropbox, Syncthing, USB drive, NFS mount).
- [ ] Sync handles 50K+ facets without blocking the daemon (V0.5-P9b — load test against the S3 adapter)
