...
- `compiled_notebook` (V0.5-P4, ADR 0019) — the AgenticOS Playbook: write-time compiled artifact pairing a `compiled_notebook` facet (recallable surface) with a `compiled_artifacts` row (rendered narrative + provenance). Sources: tagged `agent_profile`, `project`, `skill`, `verification_checklist` facets via `metadata.compile_into = [target]`. The target contract is documented as ordinary `workflow` or `skill` facet metadata with required `target`, `task`, `artifact_type`, and `quality_bar` keys; duplicate target descriptors are a context-check failure, not a daemon write rejection. Three new MCP tools (`register_compiled_artifact`, `get_compiled_artifact`, `list_compile_sources`) and parallel REST routes ship under `read:compiled_notebook` / `write:compiled_notebook` scopes. The compiler is out-of-process per ADR 0019 §Boundary statement — Tessera stores; the caller compiles. Ship-gated on V0.5-P8 (audit chain) per ADR 0019 §Rationale (8); cleared by the `test_chain_full_walk_clean_with_compiled_artifacts` security test that verifies the chain walks across `facet_inserted` + `compiled_artifact_registered` rows.
...
- `verification_checklist` and `retrospective` (V0.5-P3, ADR 0018) — the pre-delivery gate an agent runs and the post-run record of how it went. Three new MCP tools (`register_checklist`, `record_retrospective`, `list_checks_for_agent`) and parallel REST routes ship under `write:verification_checklist`, `write:retrospective`, and `read:verification_checklist` scopes. SWCR augments the candidate set with the most recent N retrospectives whose `agent_ref` matches an `agent_profile` candidate (default `retrospective_window=3`); the augmentation is closed-form, deterministic, and filtered by the calling agent's id.
- `automation` (V0.5-P5, ADR 0020) — the storage-only registry of scheduled-or-triggered work that caller-side runners (Claude Code `/schedule`, OpenClaw HEARTBEAT, cron, systemd, GitHub Actions, custom shell loops) execute. Closed metadata `{agent_ref, trigger_spec, cadence, runner}` plus optional `{last_run, last_result}`. Two new MCP tools (`register_automation`, `record_automation_run`) and parallel REST routes (`POST /api/v1/automations`, `POST /api/v1/automations/<external_id>/runs`) ship under `read:automation` / `write:automation` scopes. Read paths reuse `recall`, `list_facets`, and `show` per ADR 0020 §Rationale 3. The MCP `capture` tool rejects raw `facet_type='automation'` writes (mirror of the V0.5-P2 `agent_profile` gate) so structured-metadata bypass is impossible. Tessera registers; runners run — ADR 0020 §Boundary statement: no scheduler runtime, no outbound trigger, no in-process timer.
...
- [x] **Stale detection wired across the three source-mutation paths (V0.5-P6, ADR 0019 §Rationale 6).** `vault.compiled.mark_stale_for_source` fires from `vault.capture.capture` (un-delete path), `vault.facets.soft_delete` (covering `forget` + `compaction.run_sweep`), and `vault.skills.update_procedure`; each flip emits one `compiled_artifact_marked_stale` audit row through the chain insert path. `compiled.get` and `list_for_agent` JOIN the paired `compiled_notebook` facet and filter `WHERE facets.is_deleted = 0` so `forget` against the facet automatically tombstones the artifact (PR #61 review M1 closed). Direct membership only — no transitive propagation. Cross-agent isolation is independently load-bearing: the staleness primitive's `agent_id` filter is the second line of defence after `_verify_sources_belong_to_agent` at the write path. New unit suite `tests/unit/test_compiled_staleness.py` (14 tests covers each mutation path, idempotency, cross-agent isolation, and the tombstone JOIN); ship-gate companion `test_chain_full_walk_clean_with_compiled_staleness` confirms the chain walks across stale events alongside `facet_inserted` + `compiled_artifact_registered` rows.
...
- [x] **BYO sync round-trip preserves audit-chain integrity (V0.5-P9 part 1 + V0.5-P9b).** V0.5-P9 part 1 shipped `tessera/sync/` with envelope encryption, signed monotonic manifest, BlobStore protocol + filesystem implementation, and snapshot-based push/pull primitives; the exit-gate scenario (`test_push_pull_round_trip_preserves_chain` in `tests/integration/test_sync_round_trip.py`) verified the chain re-verifies on the restored vault. V0.5-P9b extends with the S3-compatible adapter (`tessera/sync/s3.py`, hand-rolled SigV4 over httpx per ADR-0022), the `tessera sync` CLI (`setup` / `status` / `push` / `pull` per ADR-0022 D4), watermark persistence in `_meta` per ADR-0022 D3, and an in-process fake-S3 round-trip integration suite (`tests/integration/test_sync_s3_round_trip.py`) re-running the V0.5-P9 part 1 exit-gate scenario byte-for-byte against the S3 backend. Both backends inherit-by-protocol from the same `BlobStore`. The S3 adapter test suite re-uses the same fake backend `tests.unit.test_sync_s3` builds — every threat-model §S6 mitigation re-runs against it. Filesystem-backed `LocalFilesystemStore` remains useful for filesystem-synced backup targets (iCloud Drive, Dropbox, Syncthing, USB drive, NFS mount).
...
- [x] **Compiler-orchestration CLI + recipe pack shipped (V0.5-P4 / V0.5-P7).** `tessera playbook {targets|sources|scaffold|register|stale|inspect}` covers target enumeration, source listing, deterministic brief generation, artifact registration, staleness tracing, and field-level inspection. The recipe pack at `docs/playbook-compiler-recipes.md` documents the Claude Code, local-LLM, and no-LLM manual authoring recipes plus the `<runner-name>/<recipe-name>@<semver-or-date>` compiler-version convention and the seven minimum artifact sections. There is intentionally no `tessera playbook compile` — Tessera stores; the caller compiles. CLI integration suite at `tests/integration/test_cli_playbook.py` covers the 23 paths through the command tree; eval-set, field-provenance, and staleness conventions live in `docs/system-design.md §Playbook eval-set contract`, `§Playbook field-level provenance`, and `§Playbook retrieval and staleness contract`.
