...
- [ ] Sync handles 50K+ facets without blocking the daemon (V0.5-P9b — load test against the S3 adapter is deferred to a follow-up; the per-protocol invariants are enforced today, the throughput characterisation under realistic vault sizes is pending real-host measurement)
